...
<body>
    <?php
        ...
        function showCities($cities){
            ...
            for($i=0; $i < count($cities[0]); $i++){ // Libellé
                $elem = array_keys($cities[0])[$i];
                echo "<th> $elem </th>";
            }   
            echo "</tr></thead><tbody>";
            foreach($cities as $city){ // ligne
                echo $city["ville"] == "PAU" ? "<tr class='card-panel green'>" : "<tr>";

                for($i=0; $i < count($city); $i++){ // cellule
                    $elem = array_keys($city)[$i];
                    echo "
                        <td>$city[$elem]</td>                      
                        ";
                }
